...
def prepare_training(optim_spec, lr, milestones, gamma, loss=None):
    ...
    return net, optimizer, lr_scheduler, device, loss_fn

def training(epochs, train_loader, model, device, optimizer, epoch_result_dir, loss_fn, PATH, lr_scheduler = None, load = False):
    model.train()
    count = 0
    loss_list = []
    loss_iter_list = []
    ...
    for epoch in tqdm(range(lastepoch, epochs)):
        for i, batch in enumerate(train_loader, 0):
                ...
                if float(config['best_val']) < val_write:
...
net, optimizer,lr_scheduler, device, loss_fn = prepare_training(optimizer, lr,milestones, gamma, loss)
training(epoch, train_loader=train_loader, model= net, device=device, optimizer=optimizer, loss_fn=loss_fn, epoch_result_dir= "/home/atreyee/Gayathri/Seeing-in-the-Dark-Pytorch/LLE/result_new/", PATH="/home/atreyee/Gayathri/Seeing-in-the-Dark-Pytorch/LLE/models/", lr_scheduler=lr_scheduler, load=True)
